<script lang="ts">
	...
	//功能详解：
	//当 searchValue 存在时：将 items 数组中的每个元素（代表一个模型）进行过滤。如果模型的 value 属性包含了用户输入的 searchValue（不区分大小写），则该模型会被包含在 filteredItems 中。
	//当 searchValue 为空时：不进行任何过滤，filteredItems 将包含 items 数组的所有元素。
	//这样，filteredItems 将始终包含根据当前搜索条件过滤后的模型列表，可以用于显示给用户选择。这个响应式语句确保了每当 searchValue 发生变化时，filteredItems 也会相应地更新。
	...
	const pullModelHandler = async () => {
		...
		console.log('pullModel', sanitizedModelTag);
